       ...
       foreach($this->rules as $a => $arr)
          {
             foreach($arr as $b => $conf)
                { 
                  $insert = mysqli_query($con,"INSERT INTO `apriori-algorithm`(`product_name`, `recoment`, `confidance`) VALUES ('$a','$b','$conf') ")or  die(mysqli_error($con));  
                }
          }
    }
...
$Apriori->process('Apriori-Algorithm-master/dataset.txt');
$Apriori->printFreqItemsets();
$Apriori->printAssociationRules();
$Apriori->saveFreqItemsets('Apriori-Algorithm-master/freqItemsets.txt');
$Apriori->saveAssociationRules('Apriori-Algorithm-master/associationRules.txt');
?>
